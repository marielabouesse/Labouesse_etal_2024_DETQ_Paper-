...
% Name: CocaineNAc_DataExtraction
% Cohort: NAc AllodLight-ctrl with opto and drugs (DETQ, cocaine)

% Feb 2023 - Marie Labouesse, marie.labouesse@gmail.com

% extracts FP data (select folder from a particular day eg day 1 vs. day2, containing multiple animals inside)
...
datatype = {'dFF','dFFwithin','ZScoredFF'}; %data analyses of interest -- remove speed if not analysing behavior
...
TRANGE = [-60 200]; % will create events for a -5 to +5 sec window (you can always plot less later)
...
%%%%%%%%%%%% DETQbaseline
DETQbaseline_yes = 1; % 1 if you do a baseline for the opto trials without DETQ (eg with VEH), 0 otherwise
%%%%%%%%%%% EXPERIMENT TYPE
ExperimentType = 'other'; % Options should be 5 characters. If other, then the trials are not defined and need to go in and change trial allocation.
BehaviorAnymaze = 0; % 0 if not, 1 if FP rig received frames from Anymaze interface and stored its data into a CSV file (1 row per frame), write 'anymaze'
Opto_Powers = {'drug1','drug2'}; % {'drug1','drug2'}
drug_name = {'drug1','drug2'}; % {'drug1','drug2'}
drug_trials_1 = [3,4,5]; % trials to keep for the analysis, drug 1 {3,4,5} 
drug_trials_2 = [7,8,9]; % trials to keep for the analysis, drug 2 [7,8,9] 
stim_duration = 2; %  seconds
stim_duration2 = [];  % in case their is a ramp or another duration of interest
stim_interval = 5; % stim every 5 min
extratime_prefirstopto = 60; % 60 sec; time to take for the overall graphs with the traces over time. how much time to take before the start of first opto stim
extratime_postlastopto = 200; % 200 sec; time to take for the overall graphs with the traces over time. how much time to take after end of last opto stim
...
nummice_set = 3;
experiment = 1; % 1 if tmeporal window with stable opto pulse, 2 if opto dose response for 10-45min
...
for nummice = 1:Nmice   
    ...
    AnimalID = ['ID_',mice_list(nummice).name(end-5:end)];   %last 4 digits of folders should be the animal name
    ...
    for s = 1; %:length(sessions)
        ...
        if done == 0 || reanalysis == 1 %if not done or if wish to reanalyze
            ...
            % first channel
            stream_name_type405A = {'x05A','05A','A05A','x405A'}; %possible names 
...
%                     epocs.Drug_time = data.epocs.U22_.offset;
                 ...
                 if isfield(data.epocs,'U11_'); % 
                    epocs.Drug_time = data.epocs.U11_.onset;
                    if length(epocs.Drug_time) == 1
                        epocs.Drug_time(2) = 28*60;
                        epocs.Drug_time(1) = 1*60;
                    end
%                     epocs.Drug_time = data.epocs.U22_.offset;
                end
                %Opto Stim amplitude
            ...
            if BehaviorAnymaze == 0 %ie videos not analyzed or analyzed later, this means the video and dFF are recorded for the same duration
...
%                     end
         ...
         %% LOW PASS FILTER OF FP DATA TO 1Hz
            ...
            % Drug injection times
            time_drug{1}=epocs.Drug_time(1);% estimate since button press is after injection
            time_drug{2}=epocs.Drug_time(2); % estimate since button press is after injection
...
%                 for colore=1:length(color_names)
                    ...
                    for o = 1:size(epocs.PulseStart_time)
%                         if o ~= 1 && o ~= 7
                            plot([epocs.PulseStart_time(o) epocs.PulseStart_time(o)],[42 48],'Color',[15, 128, 242]/255,'Linewidth',1)
                            plot([epocs.PulseStop_time(o) epocs.PulseStop_time(o)],[42 48],'Color',[15, 128, 242]./255,'Linewidth',1)
%                         end
                    end
                    text(3500,45,'opto stim','FontSize',16,'Color',[15, 128, 242]/255)

                    min_x=-200; max_x=4000; min_y=20; max_y=120; axis([min_x max_x min_y max_y]); 
                    axis([min_x max_x min_y max_y]); %ie same as other subplot
                    
                    xline(time_drug{1}-60,':k'); xline(time_drug{1}+120,':k',drug_name{1},'FontSize',16); 
                    xline(time_drug{2}-60,':k');  xline(time_drug{2}+120,':k',drug_name{2},'FontSize',16); 
                    %plot the rotarod area
                    error_area_onlyrectangle([time_drug{1}-60 time_drug{1}+120],[40 120],[80 80],[165, 167, 168]./255,0.8); %t_trials is the time vector 
                    error_area_onlyrectangle([time_drug{2}-60 time_drug{2}+120],[40 120],[80 80],[165, 167, 168]./255,0.8); %t_trials is the time vector 
...
%                     end
            ...
            calc_coeff_fit_465 = polyfit(streams.rawdata.(channel_names{channel}).c405(1:(time_drug{2}-60)./dt_ds),streams.rawdata.(channel_names{channel}).c465(1:(time_drug{2}-60)./dt_ds),1); %fitting control Dv2 against signal Dv1 using least squares fit of degree 1
            ...
            for d=1:length(dFF_names)
                ...
                L=legend('dFF','Stim on','Stim off'); L.Location = 'Best';
            ...
            dt_dmin = 1/sampling_rate_dmin; % delta t or unit of time (in min) per sample --> for 101.73 Hz sampling rate (after downsampling), should be close to 10 ms (9.8 ms)
            ...
            %% DETRENDING dFF
            if detrending == 1;% %% Detrend dFF, Option 1: normal detrend (other detrend options not here)
                for d=1:length(dFF_names)
            ...
            if deletemovingwindow == 1;
                for d=1:length(dFF_names)
                    ...
                    min_x=-30; max_x=max(time_vect)+30; min_y=min(streams.dFF.(dFF_names{d}))-10; max_y=max(streams.dFF.(dFF_names{d}))+10; axis([min_x max_x min_y max_y]); 
                    xlabel('time (sec)'); ylabel('dFF'); title([dFF_names{d},' Normalized data, 8th percentile removed']);
...
%             t_opto = [epocs.(Events2Align2{1}) epocs.(Events2Align2{2})]; % t_opto: a double column with all trials: column 1 (all rows) the start of the opto events and column 2 (all rows) the end
            ...
            % first look through the list of trials based on the power or frequency and group those that go together into a list of trial numbers in trial_select_820, etc
            
            if ExperimentType == 'other'
                if DETQbaseline_yes == 0 % only one group/condition
                    for d=1:length(dFF_names)
                        for o = 1:length(Opto_Powers)
                            t_opto2.(dFF_names{d}).(Opto_Powers{o}) = t_opto; % t_opto2: group the ones to average from t_opto 
                        end
                    end
                elseif DETQbaseline_yes == 1    % at least 2 conditions, eg VEH then DETQ
                     for d=1:length(dFF_names)
%                         for o = 1:length(Opto_Powers)
                            t_opto2.(dFF_names{d}).(drug_name{1}) = t_opto(drug_trials_1(1):drug_trials_1(length(drug_trials_1)),:); % t_opto2: group the ones to average from t_opto 
                            t_opto2.(dFF_names{d}).(drug_name{2}) = t_opto(drug_trials_2(1):drug_trials_2(length(drug_trials_2)),:); % t_opto2: group the ones to average from t_opto 
%                         end
                     end
                end
            end
            ...
            %% Generate matrix with the raw data aligned to the first opto stim after the drug injection minus 60 sec.
            ...
            for d=1:length(dFF_names)
                for k=1:length(datatype)
                    for pow = 1:length(Opto_Powers)
                        firstopto_after_drug = round((t_opto2.(dFF_names{d}).(Opto_Powers{pow})(1,1)-extratime_prefirstopto)./dt_ds); % this means we only take the first from the trials we took in analysis (eg for DETQ, only after 10 min post injection)
                        lastdatapoint = round(firstopto_after_drug + round(((max([length(drug_trials_1),length(drug_trials_2)])-1)*(stim_interval*60)+extratime_postlastopto)./dt_ds));
            ...
            for d=1:length(dFF_names)
               ...
               % axis
                    ...
                    ax = gca;
                    set(gca,'TickLength',[0 0])
                    L=legend(drug_name{1},drug_name{2},drug_name{2}); L.Location = 'Best';
        ...
        %                     end
            ...
            for d=1:length(dFF_names)
                ...
                if isfield(limits2plot.raw.(dFF_names{d}),'ZScoredFF')
                    limits2plot.raw.(dFF_names{d}).ZScoredFF = [-4 15];
                    limits2plot.baselinecorr.(dFF_names{d}).ZScoredFF = [-4 15];
                end
                if isfield(limits2plot.raw.(dFF_names{d}),'dFF')
                    limits2plot.raw.(dFF_names{d}).ZScoredFF = [-4 8];
                    limits2plot.baselinecorr.(dFF_names{d}).ZScoredFF = [-4 8];
                end
                if isfield(limits2plot.raw.(dFF_names{d}),'dFFwithin')
                    limits2plot.raw.(dFF_names{d}).speed = [-4 8];
...
%                             ylim([-5 8])
            ...
            for d=1:length(dFF_names)
                ...
                for i=1:length(datatype)
                    ...
                    %         tmp_avg2 = nanmean(Stim_data.OptoStim.(dFF_names{d}).(Opto_Powers{pow}).(dFFtype{i+1}),1);
                    %         tmp_error2 = nanstd(Stim_data.OptoStim.(dFF_names{d}).(Opto_Powers{pow}).(dFFtype{i+1}),1,1)./...
                    %             sqrt(sum(~isnan(Stim_data.OptoStim.(dFF_names{d}).(Opto_Powers{pow}).(dFFtype{i+1})(:,1))));
                    %         error_area(t_trials,tmp_avg2,tmp_error2,color2plot{pow+1},0.25)
                    % %         max_avg(pow) = max(tmp_avg);
                        end
...
%                         saveplot or not
            ...
            for d=1:length(dFF_names)
                for r = 2; %1:length(raw_or_corr)
                    for i=2; %1:length(datatype) % dFF within
                        figure;
                        if show_plot == 0
                            set(gcf,'visible','off')
                        end
...
%                             subplot(length(Opto_Powers),1,pow);
                            alltrials_array = [];
                            indivdata = [IndivStim_data.(raw_or_corr{r}).(dFF_names{d}).(drug_name{1}).(datatype{i});...
                                IndivStim_data.(raw_or_corr{r}).(dFF_names{d}).(drug_name{2}).(datatype{i})];
                            numrows = size(indivdata,1);
%                             Color_scale = [-1 max(max(indivdata,[],1),[],2)+1];
                            Color_scale = [0 15];
...
%                             xlim([t_trials(1) t_trials(end)])
                            xlim([-10 20])
                            if DETQbaseline_yes ==1
                                xlimits = get(gca,'XLim');
                                plot(xlimits,[3.5 3.5],'k','LineWidth',1,'LineStyle','-')
                            end
                            xlabel('Time (s)')
                            ylabel('Cocaine    Saline   ')
%                             set(gca,'YTick',[]);
                            set(gca,'Yticklabel',[]);
%                             set(gca,'YLabel',[]);
...
%                                 time2plot = [5 10 15 20 25 5 10 15 20 25 30 35 40 45 50 55 60];

                                text(-9.5,q,{time2plot},'Color','white','FontSize',12);
                                text(-7.5,q,'min','Color','white','FontSize',12);
%                                                               text(-14.5,q,{time2plot},'Color','black','FontSize',12);
%                                 text(-12.5,q,'min','Color','black','FontSize',12);

                            end
%                             annotation(gcf,'textbox',...
%                             [-0.000396825396825391 0.685507936507942 0.0809523809523809 0.0640211640211641],...
%                             'String',{'D','E','T','Q'},...
%                             'FitBoxToText','off',...
%                             'EdgeColor','none');
%                             annotation(gcf,'textbox',...
%                                 [-0.00158730158730159 0.870428571428576 0.0809523809523809 0.064021164021164],...
%                                 'String',['V',sprintf('\n'),'E',sprintf('\n'),'H'],...
%                                 'FitBoxToText','off',...
%                                 'EdgeColor','none');
...
%                         sgtitle([dFF_names{d}]);
        ...
        if done == 0 || overwrite == 1
            ...
            save([PATH2SAVE,'IndividualData.mat'],'Params','IndivStim_data','resample_Position','epocs','t_trials','datatype','dFF_names','Opto_Powers','Events2Align2','BASELINE_WIN',...
                            'stim_duration','TRANGE','dt_ds','sampling_rate_ds','length_data','streams','time_vect','streams_aligned','drug_name','time_vect_aligned');
%             end
