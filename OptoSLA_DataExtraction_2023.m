...
lowpassfiltering = 1; % 1 if you want to low pass filter anything faster than this frequency- typically yes- 0 for no
...
for nummice = 1:Nmice   
    ...
    AnimalID = ['ID_',mice_list(nummice).name(8:11)];   %last 5 digits of folders should be the animal name
    ...
    for s = 1; %:length(sessions)
        ...
        %%%%%%%%%%%% TRIALS BY DRUG
        VEH_trial1 = 1; VEH_trialend = 10; DETQ_trial1 = 11; DETQ_trialend = 21; % how the different trials were organized relative to drug injection; if cropping 180 sec
         % VEH_trial1 = 1; VEH_trialend = 8; DETQ_trial1 = 9; DETQ_trialend = 19; % how the different trials were organized relative to drug injection; if cropping 540 sec
...
%                     end
            ...
            % OPTION TO apply polyfit on VEH section, starting from 2 min
            % until 23 min (14 minutes)
            calc_coeff_fit_465 = polyfit(streams.rawdata.(channel_names{channel}).c405(round(120/dt_ds):round(1400/dt_ds)),streams.rawdata.(channel_names{channel}).c465(round(120/dt_ds):round(1400/dt_ds)),1); %fitting control Dv2 against signal Dv1 using least squares fit of degree 1
            ...
            time_drug.VEH=epocs.Drug_time(1)-60;
            time_drug.DETQ=epocs.Drug_time(1)+60; % this is the approximate time at which we were done injecting the drug
            end
...
%                         set(gcf,'Position',scrsz);
            ...
            %% Average plot of all individual trials per type of trials with all types of trials on same graph; selected trials 10-20min postdrug or defined window to get an idea
            for d=1:length(dFF_names)
                %RAW
                color2plot = {'b','g','m','r'};
                for i=2; %1:length(datatype)
                    for r=2; %1:length(raw_or_corr)
                        ...
                        hold on
                        for pow = 1:length(Opto_Powers)
                            if pow == 1; ix=3:10; else ix=3:10; end % trials 10 to 20 min, i.e. trials 3 to 5
                        ...
                        sgtitle([raw_or_corr{r},' ',datatype{i},' ',dFF_names{d},' selected trials postdrug'],'Interpreter','none')
