...
% functions needed: error_area_onlyrectangle, TDTbin2mat, SEV2mat, error_area
...
%%%%%%%%%%% PREPROCESSING
% trimming
timetrim_start_set = 600; %time to chop off at start of recording, in seconds: adjust this manually, eg 60 seconds
...
for nummice = 1:Nmice   
    ...
    AnimalID = ['ID_',mice_list(nummice).name(11:14)];   %last 5 digits of folders should be the animal name
    ...
    %                     fieldnames(data.streams) %to get directly the field names in data.streams
            ...
            %% Extract TTL data (epocs) for opto stim and camera frames % NOT RELEVANT FOR THIS EXPERIMENT
...
%calc_coeff_fit_465 = polyfit(streams.rawdata.(channel_names{channel}).c405(1:epocs.Drug_time(2)),streams.rawdata.(channel_names{channel}).c465(1:epocs.Drug_time(2)),1); %fitting control Dv2 against signal Dv1 using least squares fit of degree 1
                ...
                for k=1:length(datatype)
                    for pow = 1:length(Opto_Powers)
                        if pow==1 % VEH
                            ...
                            streams_aligned.(datatype{k}).(dFF_names{d}).(Opto_Powers{pow}) = ones(1,round(2700./dt_ds)-round(660./dt_ds))*nan; %
                            time_vect_aligned.(Opto_Powers{pow}) = ones(1,size(streams_aligned.(datatype{k}).(dFF_names{d}).(Opto_Powers{pow}),2))*nan;
                        end
                    end
                end
                ...
                for k=1:length(datatype)
                    for pow = 1:length(Opto_Powers)
                        if pow==1 % VEH
                            ...
                            streams_aligned.(datatype{k}).(dFF_names{d}).(Opto_Powers{pow}) = streams.(datatype{k}).(dFF_names{d})(round(660./dt_ds):round(2700./dt_ds));
                            time_vect_aligned.(Opto_Powers{pow})= time_vect(round(660./dt_ds):round(2700./dt_ds));  
                        end                       
                    end
                end
            end
