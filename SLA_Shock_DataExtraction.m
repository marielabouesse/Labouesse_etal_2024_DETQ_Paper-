...
% Cohort: dLight1.3b or GCAMP with shock
...
%%%%%%%%%%% PREPROCESSING
% trimming
timetrim_start_set = 300; %time to chop off at start of recording, in seconds: adjust this manually, eg 60 seconds
...
for nummice = 1:Nmice   
    ...
    AnimalID = ['ID_',mice_list(nummice).name(1:4)];   %last 4 digits of folders should be the animal name
    ...
    %                     fieldnames(data.streams) %to get directly the field names in data.streams
            ...
            if ttl_extract == 'default' 
                ...
                % Overall opto pulse
                if isfield(data.epocs,'Shok');
                    epocs.PulseStart_time = data.epocs.Shok.onset(1:4); % 4 trials
                    epocs.PulseStop_time = data.epocs.Shok.offset(1:4);
                end
...
%                         saveplot or not
            ...
            orderedmap = 0; % 0 to plot in normal order, 1 if you want to plot in the order of deg inhibition / excitation by sorting on the minimal value between 3 and 10 sec (for 10 sec stim...)
            for d=1:length(dFF_names)
                for r = 1:length(raw_or_corr)
                    for i=1:length(datatype)
                            Color_scale = [-1 3];
...
%                             subplot(length(Opto_Powers),1,pow);
                            ...
                            % if you want each animal to have its trials
                            % sorted by order in terms of deg
